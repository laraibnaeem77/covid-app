...
def save_assessment(data: dict):
    ...
    try:
        success = storage.save_assessment(data)
        if success:
            return {"status": "success", "message": "Assessment saved successfully"}
        else:
            return {"status": "error", "message": "Failed to save assessment"}, 500
    except Exception as e:
        return {"status": "error", "message": str(e)}, 500

@app.get("/assessments")
def get_assessments(limit: int = 100):
    \"\"\"Get saved assessments\"\"\"
    return {
        "assessments": storage.get_all_assessments(limit),
        "total": len(storage.get_all_assessments(limit))
    }

@app.get("/assessment-stats")
def get_assessment_stats():
    \"\"\"Get statistics from saved assessments\"\"\"
    return storage.get_assessment_stats()
